...
# Set up dependencies
ENV PACKAGES curl make libc-dev bash gcc linux-headers eudev-dev git
...
RUN GO111MODULE=on go get github.com/rakyll/statik
...
# Copy over python lib
COPY --from=build-deps /opt/pybuild/venv /opt/venv

RUN apk add --no-cache --virtual libressl libressl-dev
